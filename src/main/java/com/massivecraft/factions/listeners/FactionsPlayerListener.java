...
public class FactionsPlayerListener implements Listener {
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (me.getAutoClaimFor() != null) {
            ...
            Util.logFactionEvent(me.getAutoClaimFor(), FLogType.CHUNK_CLAIMS, me.getName(), CC.GreenB + "CLAIMED", String.valueOf(1), (new FLocation(player.getLocation())).formatXAndZ(","));
                ...
                if (FactionsPlugin.getInstance().version != 7) {
                    Bukkit.getScheduler().runTaskLater(FactionsPlugin.getInstance(), () -> {
                        if (FactionsPlugin.getInstance().version != 8) {
                            me.getPlayer().sendTitle(Util.color(finalTitle), Util.color(finalsubTitle), FactionsPlugin.getInstance().getConfig().getInt("Title.Options.FadeInTime"),
                            ...
                            me.getPlayer().sendTitle(Util.color(finalTitle), Util.color(finalsubTitle));
                    }, 5);
                }
            }
        }
