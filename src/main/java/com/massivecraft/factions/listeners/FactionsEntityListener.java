    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (event instanceof EntityDamageByEntityEvent) {
            ...
            if (damageee instanceof Player) {
                ...
                if (damageee.getType() == EntityType.ITEM_FRAME || damageee.getType() == EntityType.ARMOR_STAND) {
                    ...
                    if (damager instanceof Player)
                        ...
                        else if (!this.checkExplosionForBlock(damager, damageee.getLocation().getBlock()))
                            event.setCancelled(true);
                    return;
                }
                if (damager instanceof Player) return;
            }
            if (damageee != null && damageee instanceof Player && (playerHurt && FactionsPlugin.instance.getConfig().getBoolean("ffly.disable-flight-on-player-damage", true) || (!playerHurt && FactionsPlugin.instance.getConfig().getBoolean("ffly.disable-flight-on-mob-damage", true)))) {
                cancelFStuckTeleport((Player) damageee);
                if (!combatList.contains(damageee.getUniqueId()))
                    combatList.add(damagee.getUniqueId());
                Bukkit.getScheduler().runTaskLater(FactionsPlugin.instance, () -> combatList.remove(damageee.getUniqueId()), 20 * FactionsPlugin.instance.getConfig().getInt("ffly.CombatFlyCooldown"));
            ...
            if (damager instanceof Player) {
                ...
                Bukkit.getScheduler().runTaskLater(FactionsPlugin.instance, () -> combatList.remove(finalDamager.getUniqueId()), 20 * FactionsPlugin.instance.getConfig().getInt("ffly.CombatFlyCooldown"));
